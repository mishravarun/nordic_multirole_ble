...
static ble_custom_c_t     m_ble_custom_c;                                                    /**< Main structure used by the Heart rate client module. */
static uint16_t           m_conn_handle_custom_c            = BLE_CONN_HANDLE_INVALID;       /**< Connection handle for the HRS central application */

static ble_custom_c_t     m_ble_custom_c_1;                                                    /**< Main structure used by the Heart rate client module. */
...
/**@brief Handles events coming from the Custom central module.
 */

static void custom_c_evt_handler(ble_custom_c_t * p_custom_c, ble_custom_c_evt_t * p_custom_c_evt, int ble_num)
{
    switch (p_custom_c_evt->evt_type)
    {
        case BLE_CUSTOM_C_EVT_DISCOVERY_COMPLETE:
        {
            if (m_conn_handle_custom_c == BLE_CONN_HANDLE_INVALID)
            {
                ret_code_t err_code;

                m_conn_handle_custom_c = p_custom_c_evt->conn_handle;
                NRF_LOG_INFO("CENTRAL: Custom Service discovered on conn_handle 0x%x\r\n",
                                m_conn_handle_custom_c);

                err_code = ble_custom_c_handles_assign(p_custom_c,
                                                    m_conn_handle_custom_c,
                                                    &p_custom_c_evt->params.peer_db);
                APP_ERROR_CHECK(err_code);

                // Initiate bonding.
                NRF_LOG_INFO("CENTRAL: initiate security");
                err_code = pm_conn_secure(m_conn_handle_custom_c, false);
                if (err_code != NRF_ERROR_INVALID_STATE)
                {
                    APP_ERROR_CHECK(err_code);
                }
                ...
                err_code = ble_custom_c_hrm_notif_enable(p_custom_c);
                APP_ERROR_CHECK(err_code);
            }
        } break; // BLE_CUSTOM_C_EVT_DISCOVERY_COMPLETE

        case BLE_CUSTOM_C_EVT_HRM_NOTIFICATION:
        {
            NRF_LOG_INFO("CENTRAL: Custom Rate %d = %d\r\n", ble_num,p_custom_c_evt->params.hrm.hr_value);
        } break; // BLE_CUSTOM_C_EVT_HRM_NOTIFICATION
...
static void on_ble_central_evt(const ble_evt_t * const p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
        {
            ...
            if ((m_conn_handle_hrs_c == BLE_CONN_HANDLE_INVALID)
              || (m_conn_handle_custom_c == BLE_CONN_HANDLE_INVALID))
            {
                NRF_LOG_INFO("CENTRAL: try to find services on conn_handle 0x%x\r\n", p_gap_evt->conn_handle);
        ...
        case BLE_GAP_EVT_DISCONNECTED:
        {
            ...
            if (p_gap_evt->conn_handle == m_conn_handle_custom_c)
            {
                m_conn_handle_custom_c = BLE_CONN_HANDLE_INVALID;
            }
            ...
            else
            {
                ...
                *  offers the same service. */
                if ((find_adv_uuid(&p_gap_evt->params.adv_report, BLE_UUID_HEART_RATE_SERVICE)&&
                     (m_conn_handle_hrs_c == BLE_CONN_HANDLE_INVALID))
                     ||(find_adv_uuid(&p_gap_evt->params.adv_report, BLE_UUID_CUSTOM_SERVICE)&&
                          (m_conn_handle_custom_c == BLE_CONN_HANDLE_INVALID))
											|| (find_adv_uuid(&p_gap_evt->params.adv_report, BLE_UUID_CUSTOM_SERVICE_3)&&
                          (m_conn_handle_custom_c == BLE_CONN_HANDLE_INVALID)))
                {
                    // Initiate connection.
                    NRF_LOG_INFO("CENTRAL: connecting ...\r\n");
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    else if ((role == BLE_GAP_ROLE_CENTRAL) || (p_ble_evt->header.evt_id == BLE_GAP_EVT_ADV_REPORT))
    {
        ...
        ble_custom_c_on_ble_evt(&m_ble_custom_c, p_ble_evt, 1);
				ble_custom_c_on_ble_evt(&m_ble_custom_c_1, p_ble_evt, 2);
...
static void custom_c_init(void)
{
    uint32_t         err_code;
    ble_custom_c_init_t custom_c_init_obj;

    custom_c_init_obj.evt_handler = custom_c_evt_handler;

    err_code = ble_custom_c_init(&m_ble_custom_c, &custom_c_init_obj, BLE_UUID_CUSTOM_SERVICE);
    APP_ERROR_CHECK(err_code);
}

static void custom_c_1_init(void)
{
    uint32_t         err_code;
    ble_custom_c_init_t custom_c_init_obj;
    ...
    err_code = ble_custom_c_init(&m_ble_custom_c_1, &custom_c_init_obj, BLE_UUID_CUSTOM_SERVICE_3);
    APP_ERROR_CHECK(err_code);
}
...
static void db_disc_handler(ble_db_discovery_evt_t * p_evt)
{
    ...
    ble_custom_on_db_disc_evt(&m_ble_custom_c, p_evt, BLE_UUID_CUSTOM_SERVICE, 1);
    ble_custom_on_db_disc_evt(&m_ble_custom_c_1, p_evt, BLE_UUID_CUSTOM_SERVICE_3, 2);
